...
module.exports = function (ChaincoinServerService) {
  ...
  return (masternodeBroadcastData) => {
    ...
    if (masternodeBroadcast.verifySignature() == false)
    {
        return;
    }

    if (masternodePing.verifySignature(masternodeBroadcast.pubKeyMasternode) == false)
    {
        return;
    }

    ChaincoinServerService.chaincoinServer.SendMessage(masternodeBroadcast);
